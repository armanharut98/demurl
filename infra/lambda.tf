...
data "aws_s3_object" "post_lambda" {
  bucket = "rcgrafbucket"
  key    = "projects/demurl/lambdas/post.zip"
}

data "aws_s3_object" "post_layer" {
  bucket = "rcgrafbucket"
  key    = "projects/demurl/lambdas/post-layer.zip"
}

data "aws_s3_object" "get_lambda" {
  bucket = "rcgrafbucket"
  key    = "projects/demurl/lambdas/get.zip"
}

data "aws_s3_object" "get_layer" {
  bucket = "rcgrafbucket"
  key    = "projects/demurl/lambdas/get-layer.zip"
}

resource "aws_lambda_layer_version" "post_layer" {
  layer_name          = "post_layer"
  s3_bucket           = "rcgrafbucket"
  s3_key              = "projects/demurl/lambdas/post-layer.zip"
...
resource "aws_lambda_layer_version" "get_layer" {
  ...
  s3_bucket           = "rcgrafbucket"
  s3_key              = "projects/demurl/lambdas/get-layer.zip"
